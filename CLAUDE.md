...
**Current shipping groups:** `affection | provision | recognition | kinetic | ordnance | injection | corruption | cataclysm | manipulation` (per the 2026-05-02 redesign in `docs/abilities.md`, Phases 1–4 landed). `injection` holds prompt-engineering attacks (DAN, sycophancy-bait, hallucinate, agentic-loop, `hallucinate` and `agentic_loop` shipped in Phase 4; the other two are deferred); `recognition` holds RLHF/fame signals (citation shipped in Phase 3; thumbs-up deferred). The earlier names were `gifts | blessings | melee | ranged | elemental | god`, `gifts`+`blessings` folded into `provision`, `melee → kinetic`, `ranged → ordnance`, `elemental → corruption`, `god → cataclysm`. `bureaucracy` was proposed and rejected (too inside-baseball); one-time bits like Antitrust Filing and OpenAI Board Drama are promoted to Mode events. Migration plan continues in `docs/abilities.md` §9 (Phases 5–7 still pending).
